...
MOCK_SITE_COORDS = {
    "Sanjay Park, India": [19.2296, 72.8711],
    "Amazon Rainforest, Brazil": [-3.4653, -62.2159],
    "Borneo Peatlands, Indonesia": [-2.3333, 113.9167]
}
DEFAULT_SITE = "Sanjay Park, India"
...
with tab1:
    ...
    if st.session_state.last_report_data:
        ...
        if data.get('is_already_registered'):
            st.warning("⚠️ **This zone is already registered by another user.**")

        # PDF Download Button
        pdf_payload = {"report_data": data}
        try:
        ...
        if st.session_state.last_pdf:
            st.download_button(
                ...
                file_name=f"Reforestation_Report_{data.get('location_name', 'site')}.pdf",
        ...
        if st.session_state.suitability != "Suitable for Reforestation":
            ...
            with st.spinner("Calculating..."):
                ...
                try:
                    ...
                    if response.status_code == 200:
                        data = response.json()
                        st.subheader(f"Estimated Sequestration: {data.get('total_sequestration_per_year_tonnes')} tonnes CO₂ / year")
                        st.text(f"Equivalent to {data.get('carbon_credits_per_year')} credits.")
                        st.json(data)
                    else:
                        st.error(f"❌ Error from backend (Status {response.status_code}):")
                        st.json(response.json())
